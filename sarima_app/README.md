...
- `Prévision dépenses.Prévision cumulée` / `Prévision recettes.Prévision cumulée`
- `Budget primitif dépense` / `Budget primitif recette` (utilisés pour la prévision annuelle initiale)
...
Si les colonnes de prévision annuelle sont présentes, l'application construit deux courbes mensuelles ventilées:
- **Budget primitif saisonnalisé** (prévision initiale de début d'année),
- **Prévision service saisonnalisée** (pondération budget primitif + ajustement en cours d'année).

La pondération est configurable dans l'app (le budget primitif peut "compter plus") et ces courbes sont comparées au réel et à SARIMA.
