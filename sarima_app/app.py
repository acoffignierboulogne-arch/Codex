...
st.title("Prévision des dépenses mensuelles - SARIMA")
...
cutoff = st.slider("Cutoff (dernier mois connu)", min_value=min_cut.to_pydatetime(), max_value=max_cut.to_pydatetime(), value=max_cut.to_pydatetime(), format="MM/YYYY")
cutoff = pd.Timestamp(cutoff).replace(day=1)
...
c1.metric("AIC", f"{fit.aic:.2f}")
c2.metric("BIC", f"{fit.bic:.2f}")
c3.metric("Log-likelihood", f"{fit.llf:.2f}")

pred_df = forecaster.forecast(fit.model_fit, start_date=cutoff + pd.offsets.MonthBegin(1), horizon=horizon)

# Prévision interne sur la zone train pour visualisation complète
...
real_pre_df = pd.DataFrame({"date": train.index, "value": train.values})
real_post_df = pd.DataFrame({"date": post_real.index, "value": post_real.values})
plot_pred_df = all_pred.copy()
plot_ci_df = pred_df.copy()

real_pre_ag = aggregate_by_period(real_pre_df, agg)
real_post_ag = aggregate_by_period(real_post_df, agg)
pred_ag = aggregate_by_period(plot_pred_df, agg)
ci_low_ag = aggregate_by_period(plot_ci_df[["date", "lower"]].rename(columns={"lower": "value"}), agg)
ci_high_ag = aggregate_by_period(plot_ci_df[["date", "upper"]].rename(columns={"upper": "value"}), agg)

fig = go.Figure()
fig.add_trace(go.Scatter(x=real_pre_ag["date"], y=real_pre_ag["value"], mode="lines", name="Réel pré-cutoff", line=dict(color="#2f6db3")))
fig.add_trace(go.Scatter(x=real_post_ag["date"], y=real_post_ag["value"], mode="lines", name="Réel post-cutoff", line=dict(color="#8eb6dd", dash="dash")))
...
fig.add_trace(go.Scatter(x=pred_ag["date"], y=pred_ag["value"], mode="lines", name="Prévision SARIMA", line=dict(color="#f07a24")))
fig.update_layout(height=460, xaxis_title="Date", yaxis_title="Montant (€)")
...
if not annual.empty:
    ...
    st.subheader("Comparaison annuelle cumulée")
    st.dataframe(styled, use_container_width=True)
else:
    st.warning("Aucune année complète commune entre réel et prévision.")

st.subheader("Grid Search SARIMA")
with st.expander("Configurer et lancer"):
    ...
    max_combo = st.number_input("Limiter à N combinaisons", 1, 1000, min(int(total), 200))
    if total > 200:
        st.warning("Le grid search peut être lent au-delà de 200 combinaisons.")
    ...
    if st.button("Lancer le Grid Search"):
        ...
        if not top:
            ...
            best = top[0]
            st.write(f"Meilleur modèle: (p,d,q)=({best['p']},{best['d']},{best['q']}), (P,D,Q)=({best['P']},{best['D']},{best['Q']})")

st.subheader("Diagnostics SARIMA")
