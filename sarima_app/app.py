...
def compute_r12_comparison(real_series: pd.Series, pred_df: pd.DataFrame, cutoff: pd.Timestamp) -> pd.DataFrame:
    """Comparatif en cumul glissant 12 mois aligné sur le mois du cutoff."""
    real_map = {d.strftime("%Y-%m"): float(v) for d, v in real_series.dropna().items()}
    pred_map = {pd.Timestamp(r.date).strftime("%Y-%m"): float(r.value) for r in pred_df.itertuples()}
    rows = []
    month = int(cutoff.month)
    years = sorted({d.year for d in real_series.dropna().index})
    for year in years:
        ...
        if covered == 12:
            abs_gap = abs(real_sum - pred_sum)
            rel_gap = 0.0 if abs(real_sum) <= 1e-9 else abs_gap / abs(real_sum) * 100
            rows.append({
                "R12 fin": end.strftime("%m/%Y"),
                "Fenêtre": f"{start.strftime('%m/%Y')} → {end.strftime('%m/%Y')}",
                "Σ Réel": real_sum,
                "Σ Prévu": pred_sum,
                "Écart absolu": abs_gap,
                "Écart relatif %": rel_gap,
            })
    return pd.DataFrame(rows)
...
with st.sidebar:
    ...
    eval_mode = st.selectbox("Mode cumul", ["Année civile", "R12"], index=0)
...
""",
    ...
    for row in brk["windows"]:
        recap_rows.append(
            {
                "Période ancienne": f"{row['prev_start'].strftime('%m/%Y')} → {row['prev_end'].strftime('%m/%Y')}",
                "Période récente": f"{row['curr_start'].strftime('%m/%Y')} → {row['curr_end'].strftime('%m/%Y')}",
                "Niveau moyen ancien": row["prev_mean"],
                "Niveau moyen récent": row["curr_mean"],
                "Δ niveau % (ancien→récent)": row["mean_shift"],
                "Δ volatilité % (ancien→récent)": row["vol_shift"],
    ...
    st.dataframe(
        recap_df.style.format(
            {
                "Niveau moyen ancien": fmt_euro,
                "Niveau moyen récent": fmt_euro,
                "Δ niveau % (ancien→récent)": "{:+.1f}%",
                "Δ volatilité % (ancien→récent)": "{:+.1f}%",
            }
        ),
...
if eval_mode == "R12":
    st.markdown("<p class='title'>Comparaison R12 glissante (fenêtres de 12 mois)</p>", unsafe_allow_html=True)
    annual = compute_r12_comparison(valid_series, all_pred, cutoff)
    if not annual.empty:
...
if not auto_selection["candidates_table"].empty:
    ...
    with st.expander("Diagnostics stationnarité, features série et résidus"):
        ...
        st.write(auto_selection["ui_text"]["explain_param_guide"])
        if "series_feature_diagnostics" in auto_selection:
            st.dataframe(auto_selection["series_feature_diagnostics"], use_container_width=True)
